...
import { uploadTomatoDetails } from '../../../../api/getTomatoDetails';
import { updateTomatoDetails } from '../../../../redux/Crops/Actions/tomatoDetails';
...
const CameraAPI = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        toast.promise(
            new Promise(async (resolve, reject) => {
                try {
                    const response = await handleUpload(file, "predict-tomato");
                    if (response.success) {
                        const timestamp = new Date().toISOString();
                        dispatch(updateTomatoDetails(response.image, response.result, timestamp));
                        const data = { image: response.image, result: response.result, timestamp };
                        const uploadData = await uploadTomatoDetails(userDetails.id, data);
    ...
    return (
        <div
            className="w-60 h-60 bg-stone-200 dark:bg-stone-700 rounded-lg flex flex-col gap-2 items-center justify-center cursor-pointer transition-colors duration-200 ease-in-out"
            onClick={() => {
                if (!isCameraActive) {
